...
def _init(height, width, mask, radius): # [1000, 1000, mask, r=5]
    # init all distances to infinity
    dists = np.full((height, width), INF, dtype=float) # INF=1e6
    # status of each pixel, ie KNOWN, BAND or UNKNOWN
    flags = mask.astype(int) * UNKNOWN # UNKNOWN=2, flags=2로 채워진 마스크
    ...
    for y, x in zip(mask_y, mask_x):
        ...
        for nb_y, nb_x in neighbors:
            ...
            # neighbor already flagged as BAND(=1)
            if flags[nb_y, nb_x] == BAND:
                continue

            # neighbor out of mask => mask contour
            if mask[nb_y, nb_x] == 0:
                flags[nb_y, nb_x] = BAND # =1
                dists[nb_y, nb_x] = 0.0
